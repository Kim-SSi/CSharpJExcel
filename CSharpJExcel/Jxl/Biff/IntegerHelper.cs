...
namespace CSharpJExcel.Jxl.Biff
	...
	public sealed class IntegerHelper
		 ...
		 * @return a four byte array
		 */
		public static byte[] getFourBytes(uint i)
			{
			byte[] bytes = new byte[4];

			int i1 = (int)i & 0xffff;
			int i2 = (int)((i & 0xffff0000) >> 16);
		 ...
		 * @param i the integer value to convert
		 */
		public static void getTwoBytes(uint i, byte[] target, int pos)
			{
			target[pos] = (byte)(i & 0xff);
			target[pos + 1] = (byte)((i & 0xff00) >> 8);
			}
		...
		public static void getFourBytes(int i,byte[] target,int pos)
			{
			getFourBytes((uint)i,target,pos);
			}

		/**
		 * Converts an unsigned integer into four bytes, and places it in the array at the
		 * specified position
		 *
		 * @param target the array which is to contain the converted data
		 * @param pos the position in the array in which to place the data
		 ...
		 */
		public static void getFourBytes(uint i, byte[] target, int pos)
			{
