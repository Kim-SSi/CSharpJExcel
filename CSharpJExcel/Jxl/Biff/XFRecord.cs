...
namespace CSharpJExcel.Jxl.Biff
	 ...
	 */
	public class XFRecord : WritableRecordData,CellFormat
		{
		/**
		 * The logger
		 */
		 ...
		 */
		private CSharpJExcel.Interop.DateFormat dateFormat;

		/**
		 * The number format for this record.  Deduced when the record is read in
		 * from a spreadsheet
		 */
		private CSharpJExcel.Interop.NumberFormat numberFormat;
		 ...
		 */
		private CSharpJExcel.Jxl.Format.Format excelFormat;
		 ...
		 */
		private const int USE_FONT = 0x4;
		private const int USE_FORMAT = 0x8;
		private const int USE_ALIGNMENT = 0x10;
		private const int USE_BORDER = 0x20;
		private const int USE_BACKGROUND = 0x40;
		private const int USE_PROTECTION = 0x80;
		private const int USE_DEFAULT_VALUE = 0xf8;

		/**
		 * The list of built in date format values
		 */
		private readonly int[] dateFormats = new int[]
				{
				0xe,
				0xf,
				0x10,
		 ...
		 */
		private readonly CSharpJExcel.Interop.DateFormat[] javaDateFormats = new CSharpJExcel.Interop.DateFormat[]
			{
			new SimpleDateFormat(CSharpJExcel.Interop.DateFormat.SHORT),
			new SimpleDateFormat(CSharpJExcel.Interop.DateFormat.MEDIUM),
		 ...
		 */
		private readonly int[] numberFormats = new int[]
				{
				0x1,
				0x2,
				0x3,
		 ...
		 */
		private readonly CSharpJExcel.Interop.NumberFormat[] javaNumberFormats = new CSharpJExcel.Interop.NumberFormat[]
				{
				new DecimalFormat("0"),
		...
		public static readonly XFType cell = new XFType();
		public static readonly XFType style = new XFType();
...
//df.setDecimalFormatSymbols(symbols);
		 ...
		 */
		protected void setXFLocked(bool l)
			{
			locked = l;
		 ...
		 */
		protected void setXFShrinkToFit(bool s)
			{
			Assert.verify(!initialized);
			shrinkToFit = s;
			usedAttributes |= USE_ALIGNMENT;
		 ...
		 */
		protected void setXFWrap(bool w)
			{
			Assert.verify(!initialized);
			wrap = w;
			usedAttributes |= USE_ALIGNMENT;
		 ...
		 * @return the format
		 */
		public CSharpJExcel.Jxl.Format.Format getFormat()
			{
			if (!formatInfoInitialized)
				{
				initializeFormatInformation();
		 ...
		 * @return the font
		 */
		public CSharpJExcel.Jxl.Format.Font getFont()
			{
			if (!formatInfoInitialized)
				{
				initializeFormatInformation();
		...
		public override int GetHashCode()
			...
			hashValue ^= leftBorder.getDescription().GetHashCode();
			hashValue ^= rightBorder.getDescription().GetHashCode();
			hashValue ^= topBorder.getDescription().GetHashCode();
			hashValue ^= bottomBorder.getDescription().GetHashCode();
