...
namespace CSharpJExcel.Jxl.Write.Biff
	 ...
	 */
	public class RowRecord : WritableRecordData
		{
		/**
		 * The logger
		 */
		//private const Logger logger = Logger.getLogger(RowRecord.class);
		 ...
		 */
		private const int maxRKValue = 0x1fffffff;

		/**
		 * The minimum integer value that can be squeezed into 30 bits
		 */
		private const int minRKValue = -0x20000000;
		...
		public override byte[] getData()
			...
			int rh = rowHeight;
			if (sheet.getSettings().getDefaultRowHeight() != SheetSettings.DEFAULT_DEFAULT_ROW_HEIGHT)
				{
		...
		public void insertColumn(int col)
			...
			System.Array.Copy(oldCells, 0, cells, 0, col);

			// Copy in the remaining cells
			System.Array.Copy(oldCells, col, cells, col + 1, numColumns - col);
			...
			numColumns = System.Math.Min(numColumns + 1, maxColumns);
			}
		...
		public void removeColumn(int col)
			...
			System.Array.Copy(oldCells, col + 1, cells, col, numColumns - (col + 1));
		 ...
		 */
		public void setGroupStart(bool value)
			{
			groupStart = value;
			}
		}
