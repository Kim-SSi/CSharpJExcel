...
namespace CSharpJExcel.Jxl.Write.Biff
	{
	/**
	 * Writes outStream a compound file
	 * 
	 * Header block is -1
	 ...
	 * of BBD blocks)
	 */
	sealed class CompoundFile : BaseCompoundFile
		{
		/**
		 * The logger
		 */
		 ...
		 */
		private Dictionary<string,ReadPropertyStorage> standardPropertySets;

		/**
		 * Structure used to store the property set and the data
		 */
		public sealed class ReadPropertyStorage
			{
			public PropertyStorage propertyStorage;
			public byte[] data;
			public int number;
		...
		// writing outStream the big block depot
		/**
		 * The current position within the bbd.  Used when writing outStream the
		 * BBD
		 */
		private int bbdPos;
		...
		/**
		 * Writes outStream the excel file in OLE compound file format
		 * 
		 * @exception IOException 
		 */
		public void write()
		...
		/**
		 * Writes outStream any additional property sets
		 */
		...
		/**
		 * Writes outStream the excel data, padding it outStream with empty bytes as
		 * necessary
		 * Also write outStream empty 
		 * 
		 * @exception IOException 
		 */
		private void writeExcelData()
			{
			excelData.writeData(outStream);
		...
		/**
		 * Write outStream the document summary data.  This is just blank
		 * 
		 * @exception IOException 
		 */
		private void writeDocumentSummaryData()
			...
			// Write outStream the summary information
			outStream.Write(padding, 0, padding.Length);
			}

		/**
		 * Write outStream the  summary data.  This is just blank
		 * 
		 * @exception IOException 
		 */
		...
		private void writeHeader()
			...
			System.Array.Copy(IDENTIFIER, 0, headerBlock, 0, IDENTIFIER.Length);
			...
			int blocksToWrite = System.Math.Min(numBigBlockDepotBlocks,
										 (BIG_BLOCK_SIZE -
			...
			// Pad outStream the rest of the header with blanks
			for (int i = pos; i < BIG_BLOCK_SIZE; i++)
				headerBlock[i] = (byte)0xff;

			outStream.Write(headerBlock,0,headerBlock.Length);

			// Write outStream the extension blocks
			pos = 0;

			for (int extBlock = 0; extBlock < numExtensionBlocks; extBlock++)
				{
				blocksToWrite = System.Math.Min(numBigBlockDepotBlocks - blocksWritten,
										 BIG_BLOCK_SIZE / 4 - 1);
			...
			if (numExtensionBlocks > 0)
				{
				// Pad outStream the rest of the extension block with blanks
				for (int i = pos; i < extensionBlockData.Length; i++)
					extensionBlockData[i] = (byte)0xff;

				outStream.Write(extensionBlockData,0,extensionBlockData.Length);
				}
			}
		...
		private void checkBbdPos()
			...
			if (bbdPos >= BIG_BLOCK_SIZE)
				{
				// Write outStream the extension block.  This will simply be the next block
				outStream.Write(bigBlockDepot, 0, bigBlockDepot.Length);
		...
		/**
		 * Writes outStream the chains for the small block depot
		 */
		private void writeSmallBlockDepotChain()
			...
			byte[] smallBlockDepotChain = new byte[numSmallBlockDepotChainBlocks * BIG_BLOCK_SIZE];
			...
			foreach (ReadPropertyStorage rps in additionalPropertySets)
				...
				if (rps.data.Length <= SMALL_BLOCK_THRESHOLD &&
					...
					// Write outStream the end of chain
			...
			outStream.Write(smallBlockDepotChain,0,smallBlockDepotChain.Length);
			}

		/**
		 * Writes outStream all the data in the small block depot
		 *
		 * @exception
		 */
		private void writeSmallBlockDepot()
			...
			byte[] smallBlockDepot = new byte[numSmallBlockDepotBlocks * BIG_BLOCK_SIZE];
			...
			foreach (ReadPropertyStorage rps in additionalPropertySets)
				...
				if (rps.data.Length <= SMALL_BLOCK_THRESHOLD)
					...
					System.Array.Copy(rps.data, 0, smallBlockDepot, pos, rps.data.Length);
					pos += length;
					}
				}

			outStream.Write(smallBlockDepot,0,smallBlockDepot.Length);
			}

		/**
		 * Writes outStream the Big Block Depot
		 * 
		 * @exception IOException 
		 */
		private void writeBigBlockDepot()
			...
			// Write outStream the extension blocks, indicating them as special blocks
			...
			// The excel data has been written.  Now write outStream the rest of it
			...
			// Write outStream the block chain for the copied property sets, if present
			writeAdditionalPropertySetBlockChains();

			if (sbdStartBlock != -2)
				{
				// Write outStream the block chain for the small block depot
				writeBlockChain(sbdStartBlock, numSmallBlockDepotBlocks);

				// Write outStream the block chain for the small block depot chain
			...
			// Pad outStream the remainder of the block
			if (bbdPos != 0)
				{
