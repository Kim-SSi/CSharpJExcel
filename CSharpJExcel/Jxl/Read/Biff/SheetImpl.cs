...
namespace CSharpJExcel.Jxl.Read.Biff
	...
	public class SheetImpl : Sheet
		...
		public ColumnInfoRecord getColumnInfo(int col)
			...
			if (!columnInfosInitialized)
				...
				foreach (ColumnInfoRecord cir in columnInfosArray)
					{
					int startcol = System.Math.Max(0, cir.getStartColumn());
					int endcol = System.Math.Min(columnInfos.Length - 1, cir.getEndColumn());
		 ...
		 */
		public void addLocalName(NameRecord nr)
			{
			if (localNames == null)
				{
