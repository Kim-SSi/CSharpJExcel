...
namespace CSharpJExcel.Jxl.Read.Biff
	...
	sealed class SheetReader
		 ...
		 */
		public ArrayList getRowProperties()
			{
		 ...
		 */
		public ArrayList getColumnInfosArray()
			{
		 ...
		 */
		public ArrayList getConditionalFormats()
			{
		...
		private void handleOutOfBoundsCells()
			...
			foreach (Cell cell in outOfBoundsCells)
				{
				resizedRows = System.Math.Max(resizedRows, cell.getRow() + 1);
				resizedCols = System.Math.Max(resizedCols, cell.getColumn() + 1);
				}
			...
			if (resizedCols > numCols || resizedRows > numRows)
				{
				Cell[,] newCells = new Cell[resizedRows, resizedCols];
				for (int row = 0; row < numRows; row++)
					{
					for (int col = 0; col < numCols; col++)
						newCells[row, col] = cells[row, col];
					for (int col = numCols; col < resizedCols; col++)
						newCells[row, col] = null;
					}

				for (int row = numRows; row < resizedRows; row++)
					{
					for (int col = numCols; col < resizedCols; col++)
						newCells[row, col] = null;
					}
				...
				//    Cell[] oldRow = cells[r];
				//    System.Array.Copy(oldRow, 0, newRow, 0, oldRow.Length);
				//    cells[r] = newRow;
				//    }
				//}
			...
			//    {
			//    Cell[,] newCells = new Cell[resizedRows,resizedCols];
			//    System.Array.Copy(cells, 0, newCells, 0, cells.Length);
			//    cells = newCells;
